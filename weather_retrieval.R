...
get_weather_data = function(airport, dates, mylogger=NA, db_conn=NA){
  ...
  for (l in weather_links){
    ...
    # Log each attempt
    if (is.function(mylogger)){
      loginfo(paste('Getting link',l),mylogger=mylogger)
    }
    ...
    weather_info = tryCatch(readLines(l)[-1], # Get String Response
                            error = function(e){
                              ...
                              if(is.function(mylogger)){loginfo(paste('Error getting',l)
                                                              ,mylogger=mylogger)} # Store Error in Log
  ...
  # Log ending time
  if(is.function(mylogger)){
    loginfo('All done!',mylogger=mylogger)
  }
...
if(interactive()){
  ...
  weather_data = get_weather_data(airport, dates, mylogger=writeToFile, db_conn=con)
